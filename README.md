...
5. Setup a gmail account (if you don't already have one) and then setup a calendar for your study
    # You need one calendar per SONA study
...
8. Create access credentials (https://developers.google.com/workspace/guides/create-credentials#oauth-client-id) 
    # You need OAuth credentials for a 'Desktop app' 
    # Follow instructions above and download the .json file to your project directory
    # Change name of .json file to 'credentials.json'
    ## This must be a single line file ##
9. Using macOS terminal (Launchpad > Other) or in VS Code (View > Terminal) enter the below code (ALWAYS copy this code WITHOUT the '>')
    > python3 pip install --upgrade google-api-python-client google-auth-httplib2 google-auth-oauthlib
    # You can use py instead of python3 but I use the latter to distinguish it from older py packages I have installed
10. Navigate to your working directory
    > cd /Users/macbook/...wherever your folder is/
    # In macOS terminal if a folder in your path has a space like /Google Drive/ you need to write it as /Google\ Drive/
...
12. Edit the sona2calendar.py file as needed 
    ...
    # Find your path to python3 by typing > which python into terminal
...
13. Edit the constants.py file as needed
    ...
    # Google Calendar ID can be found via Settings and sharing> Integrate Calendar
    # You can add more studies to this part, just use a comma to separate studies
...
14. Run the script 
    ...
    # It might ask you to sign in to Google this first time, but that login will then be saved in token.json
...
BUT you can only get it to update by maually running the code in terminal every now and then like some kind of mid-90's hackerman 
...
Go to https://crontab.guru/ and play around with the tool there to find the right option for you. Note; running every minute or every 5 minutes can be intense for your computer and your internet connection. You might only want every hour or so, which would be [0  *   *   *   *]
--------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
17. In the Nano editor screen that has now appeared type the following (substitute in your own crontab code, the example here is hourly)
    ...
    # Use tabs to separate everything, not spaces
...
21. Select the number of the message you want to read by typing the number, e.g.,
    > 2
    # Continue checking emails until it works, if no error messages are in your email then it is working. You can also sign up as a participant in SONA using a dummy account and test if your dummy bookings appear in the google calendar.
22. Exit mail (^Z)
...
CONGRATULATIONS you did it. So now sit back and relax and watch your SONA appointments effortlessly appear in your Google calendar. A few notes;
...
is to add crednetials as a dictionary inside the python code... Just google it.
